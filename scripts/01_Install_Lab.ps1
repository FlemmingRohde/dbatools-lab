...
Install-DbaInstance -Version 2019 -SqlInstance $config.ComputerName -Feature Engine -Path (Join-Path $config.InstallMediaPath '2019') -AuthenticationMode Mixed

# Install SQL Server 2017 as a named instance
Install-DbaInstance -Version 2017 -SqlInstance "$($config.ComputerName)\sql2017" -Feature Engine -Path (Join-Path $config.InstallMediaPath '2017') -AuthenticationMode Mixed
